...
const blankService = (): Service => ({ icon: "Wrench", title: "", description: "", fullDescription: "", faq: "", items: [] });
...
export default function Admin() {
  ...
  const save = async () => {
    if (items.some((item) => !item.title.trim() || !item.description.trim())) {
      setMessage("Każda usługa musi mieć tytuł i krótki opis.");
      return;
    }
    setSaving(true);
    setMessage("");
    try {
      ...
      setDirty(false);
      setMessage("Usługi zostały zapisane. Zmiany są widoczne na stronie oferty i na nowej stronie głównej.");
    } catch (error) {
    ...
    return (
      <main className="grid min-h-screen place-items-center bg-[#070807] p-6 text-white">
        <div className="w-full max-w-lg border border-[#dca92c]/20 bg-[#0b0c0b] p-8 text-center">
          ...
          <p className="mt-3 text-sm leading-6 text-white/40">Zarządzanie usługami jest dostępne po zalogowaniu do panelu administratora.</p>
  ...
  return (
    <AdminPanelShell activePath="/admin/uslugi" title="Zarządzanie usługami" eyebrow="Oferta warsztatu" previewHref="/uslugi">
      <div className="mx-auto max-w-[1500px] px-4 py-7 sm:px-6 lg:px-8 lg:py-10">
        <div className="flex flex-col justify-between gap-6 border-b border-white/10 pb-7 lg:flex-row lg:items-end">
          <div>
            <div className="text-[10px] font-extrabold uppercase tracking-[0.24em] text-[#e0ad31]">Usługi</div>
            <h1 className="mt-3 font-serif text-4xl font-semibold sm:text-5xl">Jedna oferta, wszystkie widoki.</h1>
            <p className="mt-4 max-w-3xl text-sm leading-7 text-white/40">Po zapisaniu pozycje z tego modułu zasilają podstronę „Usługi” oraz kafelki usług na nowej stronie głównej. Kolejność w panelu jest kolejnością publikacji.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button onClick={() => patchItems([...items, blankService()])} className="inline-flex items-center gap-2 border border-[#dca92c]/35 px-5 py-3 text-xs font-bold text-[#e0ad31]"><Plus className="h-4 w-4" />Dodaj usługę</button>
        ...
        <div className="mt-6 flex items-center gap-3 border border-white/10 bg-[#0b0c0b] px-4 py-3">
          <Search className="h-4 w-4 text-[#e0ad31]" />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Szukaj usługi po nazwie lub opisie…" className="min-w-0 flex-1 bg-transparent text-sm text-white outline-none placeholder:text-white/25" />
        </div>
              ...
              <section key={`${index}-${service.title}`} className="border border-white/10 bg-[#0b0c0b]">
                <div className="flex flex-col gap-4 border-b border-white/10 p-5 sm:flex-row sm:items-center sm:justify-between sm:px-7">
                  <div className="flex min-w-0 items-center gap-4">
                    <div className="grid h-11 w-11 shrink-0 place-items-center border border-[#dca92c]/25 bg-[#dca92c]/[0.05] text-[#e0ad31]"><Icon className="h-5 w-5" /></div>
                    <div className="min-w-0"><div className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#e0ad31]">Usługa {index + 1}</div><div className="mt-1 truncate font-bold">{service.title || "Nowa usługa"}</div></div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button onClick={() => move(index, -1)} disabled={index === 0} className="border border-white/10 p-2 text-white/45 disabled:opacity-20" title="Przesuń wyżej"><ArrowUp className="h-4 w-4" /></button>
                    <button onClick={() => move(index, 1)} disabled={index === items.length - 1} className="border border-white/10 p-2 text-white/45 disabled:opacity-20" title="Przesuń niżej"><ArrowDown className="h-4 w-4" /></button>
                    <button onClick={() => patchItems(items.filter((_, itemIndex) => itemIndex !== index))} className="inline-flex items-center gap-2 border border-red-500/15 px-3 py-2 text-xs text-red-300"><Trash2 className="h-4 w-4" />Usuń</button>
                  </div>
                </div>

                <div className="grid gap-5 p-5 sm:p-7 lg:grid-cols-2">
                  <Field label="Tytuł"><input value={service.title} onChange={(e) => update(index, { ...service, title: e.target.value })} className={fieldClass} placeholder="Np. Diagnostyka komputerowa" /></Field>
                  ...
                  <div className="lg:col-span-2"><Field label="FAQ / informacje dodatkowe" hint="format: Pytanie | Odpowiedź"><textarea value={service.faq || ""} onChange={(e) => update(index, { ...service, faq: e.target.value })} className={`${textareaClass} min-h-28`} placeholder={"Czy trzeba zostawić auto? | Zależy od zakresu diagnostyki.\nIle trwa usługa? | Termin ustalamy indywidualnie."} /></Field></div>
                </div>
              </section>
            );
          })}
        ...
        <div className="sticky bottom-4 z-20 mt-6 flex flex-col gap-3 border border-white/10 bg-[#090a09]/95 p-4 shadow-[0_20px_60px_rgba(0,0,0,.45)] backdrop-blur-xl sm:flex-row sm:items-center sm:justify-between">
          <div className="text-xs text-white/35">{dirty ? "Masz niezapisane zmiany w ofercie." : "Wszystkie zmiany są zapisane."}</div>
          <button onClick={save} disabled={saving || !dirty} className="inline-flex items-center justify-center gap-2 bg-[#e0ad31] px-6 py-3 text-sm font-extrabold text-black disabled:opacity-35"><Save className="h-4 w-4" />{saving ? "Zapisywanie…" : "Zapisz usługi"}</button>
        </div>
